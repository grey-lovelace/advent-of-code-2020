...
(0..99).each{
    println data
    println current
    def range = [data.indexOf(current)+1,data.indexOf(current)+3].collect{it >= data.size() ? it - data.size() : it}
    ...
    current = current == data[-1] ? data[0] : data[data.indexOf(current)+1]
}
println "Answer is ${(data[(data.indexOf(1)+1)..-1] + data[0..(data.indexOf(1)-1)]).join()}"